package org.fundaciobit.pluginsib.signatureserver.api;
 ...
 *
 */
public abstract class AbstractSignatureServerPlugin extends AbstractPluginPropertiesTranslations
        implements ISignatureServerPlugin {
     ...
     * @return
     */
    public static String checkFilter(ISignaturePlugin plugin, SignaturesSet signaturesSet, boolean suportXAdES_T,
            Logger log) {
    ...
    public static String checkFilter(ISignaturePlugin plugin, final FileInfoSignature[] aFirmar, boolean suportXAdES_T,
            Logger log) {
        ...
        if (numberMax != null) {
            if (aFirmar.length > numberMax) {
                //         XYZ ZZ TODO FALTA TRADUIR
                return "Aquets plugin només pot processar " + numberMax + " firmes per transacció i se n'han rebut "
                        + aFirmar.length;
            }
        }
            ...
            if (fis.isUserRequiresTimeStamp()) {
                ...
                if (!suportXAdES_T) {
                    if (FileInfoSignature.SIGN_TYPE_XADES.equals(signType)) {
                        log.info("Exclos plugin [" + plugin.getName(new Locale("ca"))
                                + "]: XAdES no suporta segellat de temps " + signType);
                ...
                if (!canDoTimestamp) {
                    ...
                    String msg = "Exclos plugin [" + plugin.getName(new Locale("ca"))
                            + "]: NO POT CREAR SEGELL DE TEMPS PER TIPUS DE FIRMA " + signType;
                    log.info(msg);
        ...
        if (anySignatureRequireRubric) {
            if ((anySignatureRequireRubricAndNotProvidesGenerator && !plugin.providesRubricGenerator())
        ...
        if (anySignatureRequireCSVStamp) {
            ...
            (anySignatureRequireCSVStampAndNotProvidesGenerator && !plugin.providesSecureVerificationCodeStamper())
                    ...
                    || (!anySignatureRequireCSVStampAndNotProvidesGenerator
                            && !plugin.acceptExternalSecureVerificationCodeStamper())) {
                // Exclude Plugin
                // TODO XYZ ZZZ Traduir
                String msg = "Exclos plugin [" + plugin.getName(new Locale("ca"))
                        + "]: NO TE GENERADOR ESTAMPACIO CSV ";
                log.info(msg);
                return msg;
            }
