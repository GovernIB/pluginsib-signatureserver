package org.fundaciobit.pluginsib.signatureserver.afirmaserver;
...
import org.fundaciobit.pluginsib.signatureserver.afirmaserver.apiws.DSSSignature;
import org.fundaciobit.pluginsib.signatureserver.afirmaserver.apiws.DSSSignatureService;
 ...
 * @author areus
 */
public class AfirmaServerIntegra224SignatureServerPlugin extends AbstractSignatureServerPlugin {
    ...
    */
    ...
    public static final String AFIRMASERVER_BASE_PROPERTIES = SIGNATURESERVER_BASE_PROPERTY + "afirmaserver.";
    ...
    public static final String SUPPORTSPADESWITHTIMESTAMP = AFIRMASERVER_BASE_PROPERTIES + "supportsPadesWithTimeStamp";
    ...
    private final Semaphore upgradeSemaphore;
    

    public AfirmaServerIntegra224SignatureServerPlugin() {
        this("", System.getProperties());
    }

    public AfirmaServerIntegra224SignatureServerPlugin(String propertyKeyBase) {
        this(propertyKeyBase, System.getProperties());
    }

    public AfirmaServerIntegra224SignatureServerPlugin(String propertyKeyBase, Properties properties) {
        super(propertyKeyBase, properties);

        // --------- Configurar Semaphoros de cridada a apis
        ...
        // ------------ Configurar Transformació per freemarker
    ...
    protected DSSSignature getApiSign() throws Exception {
        ...
        try {
            ...
            DSSSignature apiSign = service.getDSSAfirmaSign();
            ...
            System.out.println("API SIGN inicialitzada en " + (System.currentTimeMillis() - start) + " ms");

            return apiSign;

        } catch (Exception e) {
            throw new Exception("Error inicialitzant API SIGN de @firma: " + e.getMessage(), e);
        }
    }

    protected org.fundaciobit.pluginsib.signatureserver.afirmaserver.validarfirmaapi.DSSSignature getApiUpgrade()
            throws Exception {
        long start = System.currentTimeMillis();
        try {
            ...
            service = new org.fundaciobit.pluginsib.signatureserver.afirmaserver.validarfirmaapi.DSSSignatureService(
                    wsdlUrl);
            org.fundaciobit.pluginsib.signatureserver.afirmaserver.validarfirmaapi.DSSSignature apiUpgrade;
            ...
            System.out.println("API UPGRADE inicialitzada en " + (System.currentTimeMillis() - start) + " ms");

            return apiUpgrade;
        } catch (Exception e) {
            throw new Exception("Error inicialitzant API UPGRADE: " + e.getMessage(), e);
        }
    }


    protected TransformersFacadeServerPlugin getTransformersFacade() throws Exception {

        return TransformersFacadeServerPlugin.getInstance(getPropertyRequired(TRANSFORMERSTEMPLATESPATH_PROPERTY));
    };
    ...
    public boolean providesTimeStampGenerator(String signType) {
        ...
        if (applicationID_TS == null || applicationID_TS.trim().length() == 0) {
            ...
            if (FileInfoSignature.SIGN_TYPE_XADES.equals(signType) || FileInfoSignature.SIGN_TYPE_CADES.equals(signType)
            ...
            // || FileInfoSignature.SIGN_TYPE_PADES.equals(signType)
                ...
                if (FileInfoSignature.SIGN_TYPE_PADES.equals(signType)) {
                    /* 
                     * NºP119057: Consulta plataforma @firma
                     * 
                     * RESPOSTA: Los técnicos de @firma han realizado pruebas con la versión 6.4.0 y con la petición 
                     * que nos había remitido para realizar una firma PADES-T con el servicio DSSAfirmaSign 
                     * y el resultado ha sido correcto. Dado que están usando un modelo federado obsoleto (v6.2.2)
                     *  se recomienda actualizar a la última versión del modelo federado v6.4.0_001, que solventa bugs anteriores
                     */
                    if ("true".equalsIgnoreCase(getProperty(SUPPORTSPADESWITHTIMESTAMP))) {
                        return true;
                    } else {
                        log.debug("El plugin [" + getName(new Locale("ca"))
                                + "] pareix que ataca a un servidor amb versió menor a v6.4.0_001 cosa que no permet la "
                                + "generació de firmes PADES-T. En cas de tenir un servidor v6.4.0_001 o superior"
                                + " li recomanam que activi la propietat " + SUPPORTSPADESWITHTIMESTAMP + "=true");
                    }
                }
            }
        }
    ...
    public String filter(SignaturesSet signaturesSet, Map<String, Object> parameters) {
        ...
        try {
            String pluginOrig = "org.fundaciobit.pluginsib.signatureserver.afirmaserver.AfirmaServerSignatureServerPlugin";
            Class<?> c = Class.forName(
                    "org.fundaciobit.pluginsib.signatureserver.afirmaserver.AfirmaServerSignatureServerPlugin");

            if (c != null) {
                return "El plugin " + this.getClass().getName() + " no és compatible amb el plugin " + pluginOrig
                        + ". Elimina la dependència org.fundaciobit.pluginsib.signatureserver:pluginsib-signatureserver-afirmaserver "
                        + " d'aquest projecte. ";
            }
        ...
        for(FileInfoSignature fis: signaturesSet.getFileInfoSignatureArray()) {
            long total = 0;
            if (fis.getFileToSign() != null) {
                total = total + fis.getFileToSign().length();
            }
            if (fis.getPreviusSignatureDetachedFile() != null) {
                total = total + fis.getPreviusSignatureDetachedFile().length();
            }
            
            if (total > MAX_FILE_SIZE) {
                String msg = "El servidor de @firma només permet realitzar peticions de firma amb documents de mida màxima "
                        + MAX_FILE_SIZE + " bytes. El documents a firmar tenen una mida de " + total + " bytes.";
                //log.error(msg);
        ...
        for (int i = 0; i < fileInfoArray.length; i++) {
            ...
            try {
                ...
                if (FileInfoSignature.SIGN_TYPE_PADES.equals(tipusFirma)) {
                    ...
                    // `{1}`
                    String msg = getTraduccio("tipusfirma.desconegut", locale, tipusFirma, this.getName(locale));
                ...
                if (pis != null) {
                    ...
                    inParams.put(DSSTagsRequest.SIGPOL_SIGNATURE_POLICY_IDENTIFIER, pis.getPolicyIdentifier());
                ...
                TransformersFacadeServerPlugin transformersFacade = getTransformersFacade();
                ...
                String param = fileInfo.getName() + " (" + i + ")[" + th.getClass().getName() + "]:" + th.getMessage();
    ...
    public byte[] upgradeSignature(byte[] signature, byte[] targetCertificate, SignatureTypeFormEnumForUpgrade typeform,
        ...
        if (dssSignatureFormat == null) {
            throw new Exception("Formato para actualizacion desconocido " + typeform.getType() + " - "
                    + typeform.getFormat() + " no soportado en este plugin.");
        }

        String appID = getPropertyRequired(
                AfirmaServerIntegra224SignatureServerPlugin.APPLICATIONID_SENSE_SEGELLLAT_DE_TEMPS_PROPERTY);
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug("SersigRes transactionId: " + serSigRes.getTransactionId());
        ...
        if (serSigRes.getSignature() == null) {
            throw new Exception("La respuesta retornada por el servicio DSSAfirmaVerify de la"
                    + " plataforma @firma no incluye la firma electrónica evolucionada al " + "formato "
                    + typeform.getType() + " - " + typeform.getFormat() + ".");
        }
    ...
    private SignatureFormatEnum convertEnum(SignatureTypeFormEnumForUpgrade typeForm) {
        switch (typeForm) {
            ...
            case PAdES_LTV:
                return SignatureFormatEnum.PAdES_LTV;
            ...
            case CAdES_T:
                return SignatureFormatEnum.CAdES_T;
            case CAdES_X:
                return SignatureFormatEnum.CAdES_X;
            case CAdES_X1:
                return SignatureFormatEnum.CAdES_X1;
            case CAdES_X2:
                return SignatureFormatEnum.CAdES_X2;
            case CAdES_XL:
                return SignatureFormatEnum.CAdES_XL;
            case CAdES_XL1:
                return SignatureFormatEnum.CAdES_XL1;
            case CAdES_XL2:
                return SignatureFormatEnum.CAdES_XL2;
            case CAdES_A:
                return SignatureFormatEnum.CAdES_A;
            ...
            case XAdES_T:
                return SignatureFormatEnum.XAdES_T;
            case XAdES_X:
                return SignatureFormatEnum.XAdES_X;
            case XAdES_X1:
                return SignatureFormatEnum.XAdES_X1;
            case XAdES_X2:
                return SignatureFormatEnum.XAdES_X2;
            case XAdES_XL:
                return SignatureFormatEnum.XAdES_XL;
            case XAdES_XL1:
                return SignatureFormatEnum.XAdES_XL1;
            case XAdES_XL2:
                return SignatureFormatEnum.XAdES_XL2;
            case XAdES_A:
                return SignatureFormatEnum.XAdES_A;
            ...
            case CAdES_LT_LEVEL:
                return SignatureFormatEnum.CAdES_XL;
            case CAdES_LTA_LEVEL:
                return SignatureFormatEnum.CAdES_A;

            case XAdES_C:
                return SignatureFormatEnum.XAdES_T; // XAdES_C;
     ...
     *
     */
    private ServerSignerResponse internalUpgradeSignature(UpgradeSignatureRequest upgSigReq) throws Exception {
        ...
        if (resultValidate != null) {
            throw new Exception(Language.getFormatResIntegra("IFWS020", new Object[] { resultValidate }));
        }
        ...
        if (!isXAdES) {
            ...
            xmlInput = transformersFacade.generateXml(inputParameters, "DSSAfirmaVerify", "verify", "1_0");
        ...
        if (isXAdES) {
            ...
            propertiesResult = transformersFacade.parseResponse(xmlOutput, "DSSAfirmaVerify", "verify", "1_0");
        }
    ...
    private static Boolean generateUpgradeSignatureRequest(UpgradeSignatureRequest upgSigReq,
            HashMap<String, Object> inputParameters) throws Exception {
        ...
        if (upgSigReq.getSignature() != null) {
            ...
            inputParameters.put("dss:SignatureObject/dss:Other/cmism:getContentStream/cmism:repositoryId",
                    upgSigReq.getSignatureRepository().getId());
            inputParameters.put("dss:SignatureObject/dss:Other/cmism:getContentStream/cmism:objectId",
                    upgSigReq.getSignatureRepository().getObject());
        }
        ...
        if (upgSigReq.getSignatureFormat() != null) {
            inputParameters.put("dss:OptionalInputs/dss:ReturnUpdatedSignature@Type",
                    upgSigReq.getSignatureFormat().getUriFormat());
        }
        if (upgSigReq.getTargetSigner() != null) {
            String encodedTargetSigner = Base64.getMimeEncoder(76, new byte[] { '\n' })
                    .encodeToString(upgSigReq.getTargetSigner());
    ...
    private static boolean incorporateSignatureImplicit(Map<String, Object> inputParameters, byte[] signature)
            throws Exception {
    ...
    */
    ...
    private String cridadaWsSign(String inputXml) throws Exception {
        DSSSignature apiSign = getApiSign();
