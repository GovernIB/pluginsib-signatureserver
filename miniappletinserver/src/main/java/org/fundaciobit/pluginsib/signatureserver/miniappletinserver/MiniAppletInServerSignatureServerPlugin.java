package org.fundaciobit.pluginsib.signatureserver.miniappletinserver;
...
public class MiniAppletInServerSignatureServerPlugin extends AbstractSignatureServerPlugin {
    ...
    public File getPropertyBasePath() {
        if (miniappletInServerBasePath == null) {
            ...
            log.info("MiniAppletInServerSignatureServerPlugin = " + miniappletInServerBasePath.getAbsolutePath());
    ...
    public SignaturesSet signDocuments(SignaturesSet signaturesSet, String timestampUrlBase,
        ...
        try {
            ...
            try {
                ...
                String msg = getTraduccio("error.fitxer.llegint", new Locale(commonInfoSignature.getLanguageUI()),
            ...
            for (int i = 0; i < fileInfoArray.length; i++) {
                ...
                try {
                    ...
                    info = MiniAppletUtils.convertLocalSignature(commonInfoSignature, fileInfo, timestampUrl,
                            certificate);
    ...
    public byte[] generateTimeStamp(String signaturesSetID, int signatureIndex, byte[] inputRequest) throws Exception {
        ...
        if (signaturesSet == null) {
            throw new Exception(" Dins la cache de SignaturesSet no hi ha cap element amb id " + signaturesSetID);
        }
        ...
        if (timeStampGen == null) {
            throw new Exception("El generador de TimeStamp per la petició amb id " + signaturesSetID + " val null");
        }
    ...
    public InfoCertificate getCertificateOfUser(String username, String lang) throws Exception {
        ...
        if (!userPath.exists()) {
            ...
            throw new Exception(getTraduccio("error.directori.noexisteix", locale, userPath.getAbsolutePath()));
        }
        ...
        if (!propsFile.exists()) {
            ...
            throw new Exception(getTraduccio("error.fitxer.noexisteix", locale, propsFile.getAbsolutePath()));
        }
        ...
        if (prop == null) {
            ...
            throw new Exception(getTraduccio("error.nopropietats", locale, propsFile.getAbsolutePath()));
        ...
        if (p12Password != null && p12Password.trim().length() != 0) {
            ...
            if (isEmpty(p12Password)) {
                ...
                throw new Exception(getTraduccio("error.nopropietatdefinida", locale, FIELD_P12PASSWORD,
            ...
            if (isEmpty(filename)) {
                // "No s'ha definit la propietat " " en el fitxer "
                throw new Exception(getTraduccio("error.nopropietatdefinida", locale, FIELD_P12FILENAME,
                        propsFile.getAbsolutePath()));
            }
            ...
            if (!p12File.exists()) {
                throw new Exception(getTraduccio("error.fitxer.noexisteix", locale, p12File.getAbsolutePath()));
            }
            ...
            if (!isEmpty(jksFilename)) {
                ...
                if (isEmpty(keyStorePassword)) {
                    // "No s'ha definit la propietat " " en el fitxer "
                    throw new Exception(getTraduccio("error.nopropietatdefinida", locale, FIELD_JKS_KEYSTORE_PASSWORD,
                            propsFile.getAbsolutePath()));
                }
                ...
                if (isEmpty(certificatePassword)) {
                    // "No s'ha definit la propietat " " en el fitxer "
                    throw new Exception(getTraduccio("error.nopropietatdefinida", locale, FIELD_JKS_CERT_PASSWORD,
                            propsFile.getAbsolutePath()));
                }
                ...
                if (isEmpty(alias)) {
                    // "No s'ha definit la propietat " " en el fitxer "
                    throw new Exception(getTraduccio("error.nopropietatdefinida", locale, FIELD_JKS_ALIAS,
                            propsFile.getAbsolutePath()));
                }
                ...
                throw new Exception("El fitxer " + propsFile.getAbsolutePath()
                        + " no conté informació correcta del keystore:\n" + "   + PKCS12: " + FIELD_P12FILENAME + ","
                        + FIELD_P12PASSWORD + "\n" + "   + JKS: " + FIELD_JKS_FILENAME + ", "
                        + FIELD_JKS_KEYSTORE_PASSWORD + ", " + FIELD_JKS_CERT_PASSWORD + ", " + FIELD_JKS_ALIAS);
    ...
    public interface InfoCertificate {
        ...
        public PublicCertificatePrivateKeyPair getPublicCertificatePrivateKeyPair(InfoCertificate cinfo)
                throws Exception;
    ...
    public static class JKSInfoCertificate implements InfoCertificate {
         ...
         * @param alias
         */
        public JKSInfoCertificate(File keyStore, String keyStorePassword, String certificatePassword, String alias) {
            super();
        ...
        public PublicCertificatePrivateKeyPair getPublicCertificatePrivateKeyPair(InfoCertificate cinfo)
            ...
            if (key instanceof PrivateKey) {
                ...
                // TODO Traduir
                throw new Exception(
                        "El certificat seleccionat (" + this.keyStore.getAbsolutePath() + ")no conté Clau Privada");
            }
        }
    }
    ...
    public byte[] upgradeSignature(byte[] signature, byte[] targetCertificate, SignatureTypeFormEnumForUpgrade typeform,
            ITimeStampGenerator timeStampGenerator, String timeStampUrl) throws Exception {
        // XYZ ZZZ
